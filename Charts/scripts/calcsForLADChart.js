...
function updateRow(index) {
    ...
    if (isNaN(wt)) { return };
    var lmca = parseFloat(document.getElementById('lad' + [index]).value);
    var bsa = calcBSA(ht, wt);
    var zscore = calcZ(bsa, lmca);
    document.getElementById('bsa' + [index]).innerHTML = bsa.toFixed(2);
    document.getElementById('ladZ' + [index]).innerHTML = zscore.toFixed(2);
} //end updateRow fx
...
function calcZ(bsa, score) {
    score = score / 10;
    var mean = (0.26108 * Math.pow(bsa, 0.37893)) - 0.02852;
    var sd = 0.01465 + (0.01996 * bsa);
