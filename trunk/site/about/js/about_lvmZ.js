...
$('#update1').click(function () {
    var ht = parseInt($('#ht1').val());
    if ( ht != '' && !isNaN(ht) && ht >= 47 && ht <= 191 ) {
        $('.info1').text(ht);
        var lms = lmsArray[ht];
        $('.lambda-1').text(lms[0]);
        $('.mu-1').text(lms[1]);
        ...
        $('.info1, .lambda-1, .mu-1, .sigma-1').text('');
    }
});

$('#update2').click(function () {
    var ht = parseInt($('#ht2').val());
    var lvm = parseFloat($('#lvm2').val())
    if ( ht != '' && !isNaN(ht) && ht >= 47 && ht <= 191 && lvm != '' && !isNaN(lvm) ) {
        var lms = lmsArray[ht];
        $('.lvm2').text(lvm);
        var l2 = lms[0]
        $('.lambda-2').text(l2);
        ...
        var s2 = lms[2]
        $('.sigma-2').text(s2);
        var numerator, denominator, zscore;
        numerator = Math.pow( lvm / m2, l2) - 1;
        denominator = l2 * s2;
        $('.numerator2').text(numerator);
        $('.denominator2').text(denominator)
        zscore = numerator / denominator;
        $('.zscore2').text(zscore);
        $('.zscore2_rounded').text(zscore.toFixed(2));
