...
for (var i = 47; i <= 191; i++) {
    var lambda = lmsArray[i][0];
    var mu = lmsArray[i][1];
    var sigma = lmsArray[i][2];

    mean.push([i, mu]);
    //upper
    up = 2 * lambda * sigma;
    up += 1;
    up = Math.pow(up, (1 / lambda)) * mu;
...
var datasets = [
{ data: upper, label: 'Upper' },
{ data: mean, label: 'Mean' },
